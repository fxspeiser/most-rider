...
# The whole repo is mounted read-only at /workspace anyway (for the
# --project-directory fault-control calls, see run_compose below) — reused
# here so M6's summary panel reads real committed benchmark data instead of
# hardcoding numbers into the frontend.
REPORTS_DIR = Path(os.environ.get("REPORTS_DIR", "/workspace/benchmarks/reports"))
...
@app.get("/api/reports")
def list_reports():
    """Names of every committed benchmark report (benchmarks/reports/*.json)
    — real data, generated by tools/analyze_run.py and friends, never
    hand-entered. M6's summary panel reads these, not hardcoded numbers."""
...
def get_report(name: str):
    if not _SAFE_REPORT_NAME.match(name):
        raise HTTPException(status_code=400, detail="invalid report name")
    path = REPORTS_DIR / f"{name}.json"
    if not path.exists():
        raise HTTPException(status_code=404, detail=f"no report named '{name}'")
    return json.loads(path.read_text())


# Mounted last so it never shadows the /api/* or /ws routes above — the
# built dashboard (apps/dashboard, ADR-0008), copied into the image at
# STATIC_DIR by apps/api-bridge/Dockerfile's multi-stage build.
